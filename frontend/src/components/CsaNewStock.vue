<!-- filepath: /C:/Users/Lee/Desktop/stock_management/frontend/src/components/CsaNewStock.vue -->
<template>
    <div class="insertincsastock">
      <div class="input">
  ...
  export default {
    ...
    setup() {
      ...
      const insertnewtool = async () => {
        try {
          const response = await axios.post('http://localhost:3000/api/insert-new-csa-stock', {
            toolname: toolname.value,
            availabletools: availabletools.value,
          });
          if (response.status === 200) {
            alertsView.value.showModel('Tool inserted in CSA Stock successfully');
            toolname.value = '';
