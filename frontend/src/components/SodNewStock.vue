<!-- filepath: /C:/Users/Lee/Desktop/stock_management/frontend/src/components/SodNewStock.vue -->
<template>
    <div class="insertinsodstock">
      <div class="input">
  ...
  export default {
    ...
    setup() {
      ...
      const insertnewtool = async () => {
        try {
          const response = await axios.post('http://localhost:3000/api/insert-new-sod-stock', {
            toolname: toolname.value,
            availabletools: availabletools.value,
          });
          if (response.status === 200) {
            alertsView.value.showModel('Tool inserted in SOD Stock successfully');
            toolname.value = '';
